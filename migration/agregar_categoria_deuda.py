"""
DeltaBalance — migration/agregar_categoria_deuda.py

Migración one-off: agrega a una base YA EXISTENTE la categoría especial de
routing "MOVIMIENTO CAPITAL · Deuda" (docs/PROXIMOS_PASOS.md — corrección
posterior a la Tarea 9 Parte B) que db/seed.sql ahora incluye para bases
nuevas — ver services/categorias_service.py CATEGORIAS_PROTEGIDAS y
ui/components/registro_transacciones.py.

Mismo motivo y mismo patrón exacto que
migration/agregar_categoria_ahorro_inversion.py: db/seed.sql usa INSERT OR
IGNORE, pero DatabaseManager.inicializar() solo lo aplica si la DB es
NUEVA (`if not ya_existia`, ver db/database.py) — una base que ya existía
antes de este cambio (como data/deltabalance.db) NUNCA va a recibir esta
fila solo por actualizar seed.sql.
...
Uso:
    ...
    python migration/agregar_categoria_deuda.py

    # 2) Aplicar de verdad contra la DB real (hace backup antes solo):
    python migration/agregar_categoria_deuda.py --confirmar
"""
...
def main() -> None:
    ...
    print(f"=== Agregar categoría 'Deuda' — {modo} ===")
